...
- (OCFMStatement*) cachedStatementForQuery:(NSString*)query {
    return [cachedStatements objectForKey:query];
}

- (void) setCachedStatement:(OCFMStatement*)statement forQuery:(NSString*)query {
...
- (BOOL) goodConnection {
    ...
    OCFMResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (id) executeQuery:(NSString *)sql arguments:(va_list)args {
    ...
    OCFMResultSet *rs = nil;
    
    int rc                  = 0x00;;
    sqlite3_stmt *pStmt     = 0x00;;
    OCFMStatement *statement  = 0x00;
    ...
    if (!statement) {
        statement = [[OCFMStatement alloc] init];
    ...
    rs = [OCFMResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (BOOL) executeUpdate:(NSString*)sql arguments:(va_list)args {
    ...
    if (shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[OCFMStatement alloc] init];
