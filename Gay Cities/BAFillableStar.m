...
@synthesize fillPercent, fillColor, bgColor, strokeColor, lineWidth;
...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
		...
		//default colors
		self.fillColor = [UIColor colorWithRed:.941 green:.902 blue:0 alpha:1];
...
-(void) fillStarInContext:(CGContextRef)context withRect:(CGRect)rect
{
	...
	float originalOrigin = rect.origin.x;
	...
	rect.size.width = originalWidth - rect.size.width;
	CGContextSetFillColorWithColor(context, [[UIColor clearColor] CGColor]);
	CGContextFillRect(context, rect);
	
	rect.size.width = originalWidth;
	rect.origin.x = originalOrigin;
	
	if (fillPercent == 0) {
		//rect.origin.x = ;
		//rect.origin.x = rect.size.height/2 - 3;

		CGContextSetFillColorWithColor(context, [[UIColor grayColor] CGColor]);
		CGContextFillEllipseInRect(context, CGRectMake(rect.size.width/2 - 5, rect.size.height/2 - 5, 10, 10));
	}
